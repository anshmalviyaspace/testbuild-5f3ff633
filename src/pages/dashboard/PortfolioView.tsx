...
function UserAvatar({ initials, avatarUrl, size = 16 }: { initials: string; avatarUrl?: string; size?: number }) {
  const dim = `w-${size} h-${size}`;
  return (
    <div className={`${dim} rounded-full overflow-hidden flex items-center justify-center shrink-0 text-lg font-heading font-bold text-primary-foreground`}
...
export default function PortfolioView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (qr) setQuiz(qr as QuizResult);
      else {
        const s = localStorage.getItem("justbuild_quiz_results");
        if (s) { const p = JSON.parse(s); setQuiz({ personality_type: p.personality_type, personality_description: p.personality_description, score: p.score, level: p.level }); }
      }
      const { data: pr } = await supabase.from("profiles").select("created_at").eq("id", currentUser.id).single();
      if (pr?.created_at) setJoined(new Date(pr.created_at).toLocaleDateString("en-US", { month: "long", year: "numeric" }));
    })();
  }, [currentUser]);

  // ── PDF export via window.print with print-only CSS ──────────────────────
  ...
  if (isLoading || !currentUser) {
    return (
      <div className="p-6 sm:p-8 max-w-4xl space-y-6">
        <Sk className="h-8 w-40" /><Sk className="h-32 w-full" /><Sk className="h-48 w-full" />
      </div>
    );
  }
    ...
    <>
      ...
      <style>{`
        @media print {
          ...
          #portfolio-print-root { display: block !important; }
      ...
      <div id="portfolio-print-root" ref={printRef} className="p-6 sm:p-8 max-w-4xl animate-fade-in opacity-0">
        ...
        {/* ── Profile card ─────────────────────────────────────────────── */}
        <section className="print-card bg-card border border-border rounded-2xl p-6 mb-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5 pt-5 border-t border-border">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider mt-0.5">Member Since</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="print-card bg-card border border-border rounded-2xl p-6 mb-5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-5">
              ...
              <div className="flex gap-6 text-center flex-wrap">
                {[
                  { label: "Starting Level", value: quiz.level, sub: "when joined" },
                  { label: "Knowledge Score", value: `${quiz.score}/100`, sub: "at assessment", accent: true },
                  { label: "Current XP", value: String(currentUser.xpPoints), sub: "earned building" },
                ].map((s) => (
                  <div key={s.label}>
        ...
        <section className="mb-5">
          ...
          {projectsLoading ? (
            <div className="grid sm:grid-cols-2 gap-4">{[1,2].map((i) => <Sk key={i} className="h-48" />)}</div>
          ) : projects.length === 0 ? (
            <div className="bg-card border border-border rounded-2xl p-8 text-center">
              <p className="text-sm text-muted-foreground mb-1">Your portfolio is empty — for now.</p>
              <Link to="/dashboard/track" className="text-xs font-mono text-primary hover:underline">Start your first project →</Link>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 gap-4">
              {projects.map((p) => (
                <div key={p.id} className="print-card bg-card border border-border rounded-2xl overflow-hidden group hover:border-muted-foreground/30 transition-colors">
                  ...
                  <div className="p-4 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {(p as any).project_url && (
                        <a href={(p as any).project_url} target="_blank" rel="noopener noreferrer"
                          onClick={(e) => e.stopPropagation()}
                          className="shrink-0 text-muted-foreground hover:text-primary transition-colors" title="View project">
        ...
        {/* ── PDF Export CTA ───────────────────────────────────────────── */}
        <div className="no-print mt-8 border-t border-border pt-6 flex items-center justify-between flex-wrap gap-4">
          <div>
            <p className="text-sm font-medium">Ready to share your portfolio?</p>
            <p className="text-xs text-muted-foreground mt-0.5">Export as PDF to attach to applications, or share your public link.</p>
          </div>
          <div className="flex items-center gap-3">
            <Link to={`/profile/${currentUser.username}`}
