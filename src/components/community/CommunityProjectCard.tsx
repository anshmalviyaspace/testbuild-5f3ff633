...
interface Props { project: CommunityProjectRow; onLike: () => void; onClick: () => void; }
...
export default function CommunityProjectCard({ project: p, onLike, onClick }: Props) {
  ...
  const handleAuthorClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (p.author_username) navigate(`/profile/${p.author_username}`);
  };

  return (
    <div onClick={onClick} className="bg-card border border-border rounded-xl overflow-hidden card-hover-glow cursor-pointer group">
      ...
      <div className="p-4 space-y-2.5">
        {/* Clickable author → public profile */}
        <button onClick={handleAuthorClick} className="flex items-center gap-2 w-full text-left group/author">
          ...
          <span className="text-xs font-mono text-muted-foreground truncate group-hover/author:text-primary transition-colors">
        ...
        <div className="flex flex-wrap gap-1">
          {p.tags.map((tag) => (
            <span key={tag} className="text-[9px] font-mono bg-surface2 text-muted-foreground px-1.5 py-0.5 rounded">{tag}</span>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between pt-1 text-[11px] font-mono text-muted-foreground">
          ...
          <button onClick={(e) => { e.stopPropagation(); onLike(); }}
            className={`flex items-center gap-1 px-2 py-1 rounded-md transition-all ${p.user_liked ? "bg-destructive/10 text-destructive" : "hover:bg-surface2 hover:text-destructive"}`}>
